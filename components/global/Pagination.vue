...
const pageCount = computed(() => Math.max(1, Math.ceil((props.totalItems || 0) / (props.pageSize || 1))))
...
const pageItems = computed<(number | 'ellipsis')[]>(() => {
  ...
  for (let i = cur - sib; i <= cur + sib; i++) if (i >= 1 && i <= pc) set.add(i)
...
function onPrev() { go(props.page - 1) }
function onNext() { go(props.page + 1) }
function onFirst() { go(1) }
function onLast() { go(pageCount.value) }
